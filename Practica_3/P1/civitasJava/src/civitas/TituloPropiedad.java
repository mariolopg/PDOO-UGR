...
public class TituloPropiedad {
     ...
     */
    TituloPropiedad(String nom, float ab, float fr, float hb, float pc, float pe, Barrio barrio) {
        nombre = nom;
        alquilerBase = ab;
    ...
    */
    ...
    Boolean vender(Jugador jugador) {
        ...
        if (esEsteElPropietario(jugador) && !getHipotecado()) {
            propietario.recibe(getPrecioVenta() - (getPrecioVenta() - getPrecioCompra()) * barrio.getPorcentajePlusvalia()/100);
            numCasas = 0;
            numHoteles = 0;
            propietario = null;
            vendido = true;
