...
public class Sorpresa {
    ...
    void aplicarAJugador(int actual, ArrayList<Jugador> todos) {
        ...
        switch (tipo) {
            ...
            case ESTRELLADO:
                aplicarAJugador_estrellado(actual, todos);
                break;
        }
    }
   ...
   */
    ...
    //NUEVO
    Sorpresa(TipoSorpresa tipo, String texto) {
        init();
        this.tipo = tipo;
        this.texto = texto;
    }
    
    private void aplicarAJugador_estrellado(int actual, ArrayList<Jugador> todos) {
        int ip = todos.get(actual).propiedadMenosValor();
        if(ip != -1){
            todos.get(actual).vender(ip);
            informe(actual, todos);
        }
    }
}
