...
module Civitas
  class Tablero
    ...
    def initialize(n_cas_carcel=1)

      if(n_cas_carcel >= 1)
        @num_casilla_carcel = n_cas_carcel
      end
      @casillas = Array.new
      casilla = Casilla.casilla_descanso("Salida")
      @casillas << casilla
      @por_salida = 0
      @tiene_juez = false
...
#      El tablero es correcto si la posicion de la carcel está dentro del tablero y si tiene juez.
      ...
      def correcto
        ok = false
        if( @casillas.size > @num_casilla_carcel && @tiene_juez == true )
          ok = true
        end
        return ok
      end
...
#          Si al añadir la casilla la siguiente casilla es la carcel, la añade.
      ...
      def aniade_casilla(c)
        ...
        casilla = Casilla.casilla_descanso("Carcel")

        if(@num_casilla_carcel == @casillas.size)
          @casillas << casilla
        end
...
#          Si el tablero no tiene casilla jue, la añade al final de éste.
      ...
      def aniade_juez
        casilla = Casilla.casilla_juez(@num_casilla_carcel, "Juez")
