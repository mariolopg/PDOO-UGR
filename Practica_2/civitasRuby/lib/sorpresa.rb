  ...
  class Sorpresa
    ...
    def aplicar_a_jugador(actual, todos)
      ...
      when Tipo_sorpresa::PORCASAHOTEL
        aplicar_a_jugador_pagar_casa_hotel(actual,todos)
    ...
    def aplicar_a_jugador_ir_a_casilla(actual, todos)
      if jugador_correcto(actual, todos)
        ...
        casilla_actual = todos.get(actual).get_num_casilla_actual()
        tirada = tablero.calcular_tirada(casilla_actual)
        nueva_pos = tablero.nueva_posicion(casilla_actual, tirada)
        todos.get(actual).mover_a_casilla(nueva_pos)
    ...
    def aplicar_a_jugador_ir_carcel(actual, todos)
      if jugador_correcto(actual, todos)
        informe(actual, todos)
        todos.get(actual).encarcelar(tablero.num_casilla_carcel())
      end
    end
    ...
    def aplicar_a_jugador_pagar_cobrar(actual, todos)
      if jugador_correcto(actual, todos)
        informe(actual, todos)
        todos.get(actual).modificar_saldo(valor)
      end
    end
    ...
    def aplicar_a_jugador_casa_hotel(actual, todos)
      if jugador_correcto(actual, todos)
        informe(actual, todos)
        saldo = valor*(todos.get(actual))
        todos.get(actual).modificar_saldo(saldo)
      end
    end
    ...
    def aplicar_a_jugador_por_jugador(actual, todos)
      if jugador_correcto(actual, todos)
        informe(actual, todos)
        Sorpresa pagar = Sorpresa.crear_otras_sorpresas(Tipo_sorpresa::PAGARCOBRAR, valor*-1, "Los jugadores pagan #{valor} al jugador #{todos.get(actual).nombre}")
        Sorpresa cobrar = Sorpresa.crear_otras_sorpresas(Tipo_sorpresa::PAGARCOBRAR, valor*(todos.size - 1), "El jugador #{todos.get(actual).nombre} recibe #{valor} del resto de jugadores")
        
        cobrar.aplicar_a_jugador_pagar_cobrar(i,todos)
    ...
    def aplicar_a_jugador_salir_carcel(actual, todos)
      if jugador_correcto(actual, todos)
        ...
        while (i<todos.size && !alguien_tiene)
          if (todos.get(i).tiene_salvoconducto() )
            alguien_tiene = true 
          end
          i+=1        
        end
        ...
        if (!alguien_tiene)
          todos.get(actual).obtener_salvoconducto(self)
        end
      end
    end
    ...
    def informe(actual, todos)
      Diario.instance.ocurre_evento("Se esta aplicando una sorpresa al jugador #{todos.get(actual).nombre}")
    end
