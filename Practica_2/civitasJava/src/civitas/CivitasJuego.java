...
public class CivitasJuego {
  ...
  */
   ...
   */
  /* Comentado para que no dé error ya que cancelar hipoteca de jugador no existe aun
   ...
   */
  /* Comentado para que no dé error ya que construir casa de jugador no existe aun
   ...
   */
/* Comentado para que no dé error ya que construir hotel de jugador no existe aun
   ...
   */
  /* Comentado para que no dé error ya que hipotecar de jugador no existe aun
  public boolean hipotecar(int ip) {
    getJugadorActual().hipotecar();
  }*/
  ...
  private void inicializarTablero(MazoSorpresas mazo) {
    ...
    tablero.añadeCasilla(new Casilla (new TituloPropiedad("Moscú", 20, (float)1.05, 100 , 200, 125)));

    tablero.añadeCasilla(new Casilla (new TituloPropiedad("Oslo", 60, (float)0.95, 150, 300, 125)));
    
    //Sorpresa
    tablero.añadeCasilla(new Casilla (mazo, "Sorpresa"));

    tablero.añadeCasilla(new Casilla (new TituloPropiedad("Manila", 50, (float)0.5, 250, 500, 250)));
  
    //Se añade automaticamente la carcel

    tablero.añadeCasilla(new Casilla (new TituloPropiedad("Marsella", 70, (float)1.5, 300, 700, 250)));
    
    //sorpresa
    tablero.añadeCasilla(new Casilla (mazo, "Sorpresa"));

    tablero.añadeCasilla(new Casilla (new TituloPropiedad("Palermo", 80, (float)0.85, 400, 960, 500)));
    
    tablero.añadeCasilla(new Casilla (new TituloPropiedad("Berlin", 100, (float)1.15, 500, 1000, 500)));
    
    //Parking
    tablero.añadeCasilla(new Casilla ("Descanso"));

    tablero.añadeCasilla(new Casilla (new TituloPropiedad("Helsinki", 120, (float)1.25, 600, 1200, 625)));
    
    tablero.añadeCasilla(new Casilla (new TituloPropiedad("Lisboa", 130, (float)0.75, 650, 1300, 625)));
   
    //impuesto
    Random random = new Random();
    int cantidad = random.nextInt(451)+50; //Impuesto random entre 50 y 500
    tablero.añadeCasilla(new Casilla(cantidad, "Impuesto"));

    tablero.añadeCasilla(new Casilla (new TituloPropiedad("Estocolmo", 150, (float)0.8, 750, 1500, 750)));
    
    //Juez
    tablero.añadeJuez();

    tablero.añadeCasilla(new Casilla (new TituloPropiedad("Denver", 170, (float)1.2, 850, 1700, 875)));
    

    tablero.añadeCasilla(new Casilla (new TituloPropiedad("Rio", 180, (float)1.1, 900, 1800, 875)));
    
    // sorpresa
    tablero.añadeCasilla(new Casilla (mazo, "Sorpresa"));

    tablero.añadeCasilla(new Casilla (new TituloPropiedad("Tokio", 200, (float)0.9, 1000, 20000, 1000)));
 ...
 */
  ...
  public static void main(String[] args) {
    ...
    System.out.println("Tablero creado: " + juego.tablero.toString());
    System.out.println("Mazo creado: " + juego.mazo.toString());

    System.out.println("Info jugador actual: " + juego.infoJugadorTexto());
    juego.pasarTurno();
    System.out.println("Info jugador actual: " + juego.infoJugadorTexto());

    System.out.println("Saldo" + juego.getJugadorActual().getSaldo());
    juego.getJugadorActual().encarcelar(juego.tablero.getCarcel());
    System.out.println("Puede salir pagando: " + juego.salirCarcelPagando()); 
    System.out.println("Saldo" + juego.getJugadorActual().getSaldo());
    
    for(int i = 0; i < 5; i++)
        System.out.println("Puede salir tirando: " + juego.salirCarcelTirando());
    
    juego.getJugadorActual().modificarSaldo((float)-7200);
    System.out.println("Saldo" + juego.getJugadorActual().getSaldo());
    System.out.println("Puede salir pagando: " + juego.salirCarcelPagando());
    
    System.out.println("Fin juego: " + juego.finalDelJuego());
    
    juego.getJugadorActual().modificarSaldo((float)-101);
    
    System.out.println("Fin juego: " + juego.finalDelJuego());
    
    System.out.println(juego.ranking().toString());
    
    System.out.println(juego.getJugadorActual().getNombre());
    System.out.println("Casilla actual: " + juego.getCasillaActual());
    juego.pasarTurno();
    System.out.println(juego.getJugadorActual().getNombre());
    System.out.println("Casilla actual: " + juego.getCasillaActual());
    juego.getJugadorActual().moverACasilla(19);
    System.out.println("Casilla actual: " + juego.getCasillaActual());
    int tirada = Dado.getInstance().tirar();
    juego.getJugadorActual().moverACasilla(juego.tablero.nuevaPosicion(19, tirada));
    System.out.println("Casilla actual: " + juego.getCasillaActual());
    System.out.println("Saldo " + juego.getJugadorActual().getSaldo());
