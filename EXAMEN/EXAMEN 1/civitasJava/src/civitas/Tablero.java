...
public class Tablero {
  ...
  int getPosicionSiguienteEstacion(int posicionActual){
      int pos = -1;
      for(int i = posicionActual + 1; i < casillas.size() && pos == -1; i++)
  ...
  void añadirEstacionADiario(int i, int pos){
      if(pos != -1)
          Diario.getInstance().ocurreEvento("La estacion mas cercana a la casilla " + casillas.get(i).toString() + " es la estacion " + casillas.get(pos).toString());
      else
          Diario.getInstance().ocurreEvento("No hay estaciones cercanas a la casilla " + casillas.get(i).toString());
  }
  ...
  void calcularFactoresComunicacion(){
      for(int i = 0; i < casillas.size()-1; i++){
          ...
          if(tipo==TipoCasilla.CALLE){
              ...
              int pos = getPosicionSiguienteEstacion(i);
              if(pos != -1)
                  calcularFactorComunicacion(i, pos);
              añadirEstacionADiario(i, pos);
          }
       }
  }
  
  private void calcularFactorComunicacion(int i, int pos){
        float factor = 0;
        Casilla casilla = casillas.get(i);
        Casilla casillaEstacion = casillas.get(pos);
        Estacion estacion = casillaEstacion.getEstacion();
        TipoEstacion tipoEstacion = estacion.getTipo();
        int distancia = pos + i;
        if(tipoEstacion == TipoEstacion.cercanias)
            factor = 1 / (distancia * 10f);
        else if(tipoEstacion == TipoEstacion.largaDistancia)
            factor = 2 / (distancia * 10f);
        else if(tipoEstacion == TipoEstacion.multiple)
            factor = 3 / (distancia * 10f);
